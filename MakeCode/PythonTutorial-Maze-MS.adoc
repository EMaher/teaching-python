...
----
<.> `move_sprite` tells the `controller` object to move `dash the duck` at a speed of `100` horizontally and `100` vertically.
...
Did Dash go off the screen?  We will fix that later.
...
Did you notice that Dash can move off the screen?  Let's fix that now by telling the scene to follow the duck as it moves through the scene.
...
scene.on_overlap_tile( <.>
    ...
    on_overlap_tile)
----
<.> The `def` keyword is how functions are defined in python.  This function is called `on_overlap_tile`. It's common, but a little confusing to have the event name match the function name.  However, you can name the function what ever you want.  The function takes two parameters: `sprite` and `location`.  The `sprite` parameter is the player sprite (i.e. `dash_the_duck`) and the `location` parameter is the tile that was overlapped.  
...
- The `on_overlap_tile` parameter is the function that will be called when the event is triggered.
...
NOTE: If you switch back to the block editor, then back to the python editor again, you will see MadeCode Arcade has renamed the function back to the original name.
